...
const decode_jwt_1 = require("./decode_jwt");
...
function validate_jwt(secret, jwt) {
    ...
    // decode JWT
    const { payload, id, expires_at } = (0, decode_jwt_1.decode_jwt)(secret, jwt);
    const [enc_header, enc_payload, enc_sign] = jwt.split('.');
    // create signature
    const new_payload = (0, b64_1.b64encode)(JSON.stringify(payload));
    const signature = crypto_js_1.default.HmacSHA256(`${enc_header}.${new_payload}`, secret).toString();
    const signb64 = (0, b64_1.b64encode)(signature);
